...
function RouteComponent() {
  ...
  const { data: transData, error: transError, isError: isTransError } = useQuery({
  ...
  const { data: corridor, error: corridorError, isError: isCorridorError } = useQuery({
  ...
  const { data: positionData, error: positionError, isError: isPositionError } = useQuery({
      ...
      <div className="flex flex-row gap-2 px-4 py-4 flex-nowrap">
        {isTransError ? (
          ...
          transData.map(item => (
            <Link
              to="."
              search={(prev) => ({ ...prev, trans: item.trans, code: item.pref, route: item.route })}
              key={item.id}
